...
public abstract class YggdrasilAuthenticationServiceMixin {
    ...
    private void identity2$guardFetchProperties(
            YggdrasilUserApiService userApiService,
        ...
        try {
            ...
            if (identity2$isAuthServiceUnavailable(throwable)) {
                Identity2.LOGGER.warn("Mojang auth service unavailable; continuing in offline mode: {}", throwable.getMessage());
                return;
            }
    ...
    private static boolean identity2$isInvalidCredentials(Throwable throwable) {
        ...
        while (current != null) {
            ...
            if (current instanceof MinecraftClientHttpException httpException && httpException.getStatus() == MinecraftClientHttpException.UNAUTHORIZED) {
                return true;
            }
