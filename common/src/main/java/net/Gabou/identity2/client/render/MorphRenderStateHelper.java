...
public final class MorphRenderStateHelper {
    ...
    private static void applySyncedAbilityState(Entity source, EntityRenderState renderState) {
        ...
        if (beamActive) {
            startAnimationStates(
                renderState,
                (int) PredefIdentityAbilities.getSyncedAnimationStartTick(source, PredefIdentityAbilities.ANIM_BEAM_TICKS_KEY),
                Set.of("beam", "sonic", "laser")
            );
        }
        if (chargeActive) {
            startAnimationStates(
                renderState,
                (int) PredefIdentityAbilities.getSyncedAnimationStartTick(source, PredefIdentityAbilities.ANIM_CHARGE_TICKS_KEY),
                Set.of("charge", "ram", "bite", "attack")
            );
            setBooleanFieldIfPresent(renderState, "isAggressive", true);
        ...
        if (jumpActive) {
            startAnimationStates(
                renderState,
                (int) PredefIdentityAbilities.getSyncedAnimationStartTick(source, PredefIdentityAbilities.ANIM_JUMP_TICKS_KEY),
                Set.of("jump", "leap", "hop")
            );
        }
        if (rollActive) {
            startAnimationStates(
                renderState,
                (int) PredefIdentityAbilities.getSyncedAnimationStartTick(source, PredefIdentityAbilities.ANIM_ROLL_TICKS_KEY),
                Set.of("roll")
            );
        ...
        if (attackActive) {
            startAnimationStates(
                renderState,
                (int) PredefIdentityAbilities.getSyncedAnimationStartTick(source, PredefIdentityAbilities.ANIM_ATTACK_TICKS_KEY),
                Set.of("attack", "bite")
            );
            setBooleanFieldIfPresent(renderState, "attacking", true);
            setBooleanFieldIfPresent(renderState, "isAggressive", true);
    ...
    private static void applyAllayHoldingState(Entity identity, EntityRenderState renderState) {
        ...
        if (livingIdentity.getMainHandItem().isEmpty()) {
            return;
        }
    ...
    private static void startAnimationStates(Object renderState, int tickCount, Set<String> names) {
        ...
        for (Class<?> current = renderState.getClass(); current != null; current = current.getSuperclass()) {
            for (Field field : current.getDeclaredFields()) {
                ...
                boolean matches = names.stream().anyMatch(normalizedName::contains);
                if (!matches) {
                    continue;
                }
                try {
                    ...
                    invokeOneArg(value, "startIfStopped", tickCount);
                } catch (Throwable ignored) {
                }
            }
        }
