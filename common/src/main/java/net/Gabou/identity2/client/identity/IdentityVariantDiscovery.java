...
public final class IdentityVariantDiscovery {
    ...
    private static void ensureDefaultVariantWhenBabyPresent(Identifier typeId, Map<String, IdentityVariant> out) {
        ...
        for (IdentityVariant variant : out.values()) {
            ...
            boolean babyByFlag = nbt.getBoolean("IsBaby").isPresent() && nbt.getBooleanOr("IsBaby", false);
            boolean babyByAlias = nbt.getBoolean("Baby").isPresent() && nbt.getBooleanOr("Baby", false);
            boolean babyByAge = nbt.getInt("Age").isPresent() && nbt.getInt("Age").get() < 0;
...
//
    ...
    private static List<IdentityVariant> discoverBabyVariantsFromExisting(
        ...
        for (IdentityVariant baseVariant : existing) {
            ...
            boolean alreadyBabyByFlag = baseNbt.contains("IsBaby") && baseNbt.getBooleanOr("IsBaby", false);
            boolean alreadyBabyByAlias = baseNbt.contains("Baby") && baseNbt.getBooleanOr("Baby", false);
            boolean alreadyBabyByAge = baseNbt.contains("Age") && baseNbt.getInt("Age").isPresent() && baseNbt.getInt("Age").get() < 0;
    ...
    private static List<IdentityVariant> discoverBabyVariants(EntityType<?> type, Identifier typeId, ClientLevel world) {
        ...
        if (!variant.getBoolean("IsBaby").isPresent() && !variant.getBoolean("Baby").isPresent()) {
    ...
    private static boolean isBlacklistedStringTag(CompoundTag source, String key) {
        ...
        String raw = source.getStringOr(key, "").trim();
        if (raw.isBlank()) {
            return false;
        }
