...
| `creeper` | Explode at current position. | None. | Primary `200` | Use carefully in melee range; press `V`. |
...
| `ender_dragon` | Launch dragon fireball toward looked point or target. | None. | Primary `120` | Aim and press `V`. |
| `enderman` | Teleport to looked location with safe-space check. | None. | Primary `100` | Look where you want to blink and press `V`. |
| `endermite` | Random short-range teleport. | None. | Primary `80` | Press `V` for random reposition. |
...
| `warden` | Sonic Boom style ranged strike with knockback. | None. | Primary `120` | Aim at a living target and press `V`. |
| `witch` | Throw random negative splash potion (harming, poison, slowness, weakness). | None. | Primary `100` | Aim and press `V`. |
