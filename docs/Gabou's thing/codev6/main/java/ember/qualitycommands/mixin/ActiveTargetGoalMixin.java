...
public class ActiveTargetGoalMixin{
	...
	private void identityBreak(CallbackInfo info) {
		QualityCommands.indexOverrideActive+=1;
		//QualityCommands.LOGGER.info("activeTargetGoal +1");
	}
	...
	private void identityFix(CallbackInfo info) {
		...
		if(((EntityAccessor)this.targetEntity).getCurrentIdentity()!=null){
			QualityCommands.LOGGER.info("Target Rerouted from "+this.targetEntity.getName().getString()+" to "+((EntityAccessor)this.targetEntity).getCurrentIdentity().getName().getString());
		}
		}
	}
